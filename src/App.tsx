...
export const App = () => {
  ...
  const [todolistsState, todolistsDispatch] = useReducer(todolistsReducer, [
  ...
  const changeFilter = (todolistId: string, filter: FilterValues) => {
    todolistsDispatch(changeTodolistFilterAC({id: todolistId, filter}))
  }
  ...
  const changeTodolistTitle = (todolistId: string, title: string) => {
    todolistsDispatch(changeTodolistTitleAC({id: todolistId, title}))
  }

  const deleteTask = (todolistId: string, taskId: string) => {
    tasksDispatch(deleteTaskAC({todolistId, taskId}))
  }

  const createTask = (todolistId: string, title: string) => {
    tasksDispatch(createTaskAC({todolistId, title}))
  }
        ...
        <div className={'app'}>
          ...
          <Container maxWidth={'lg'}>
            <Grid container sx={{mb: '30px'}}>
              <CreateItemForm onCreateItem={createTodolist} placeholder='todolist'/>
            </Grid>
            <Grid container spacing={4}>
              {todolistsState?.map(todolist => {
                const todolistTasks = tasksState[todolist.id]
